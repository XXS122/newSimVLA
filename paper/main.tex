...
\subsection{动作变分自编码器（ActionVAE）}
\label{sec:method:vae}

\subsubsection{动机}

流匹配直接在原始动作空间 $\mathbb{R}^{T \times D_a}$（$T=10$，$D_a=7$）操作，该空间维度为 70。在长时序任务中，策略需跨越多个阶段（拿起→移动→放置），动作块方差大，直接建模困难。受 RoLD~\cite{rold} 启发，我们将动作块压缩至低维隐变量 $\mathbf{z} \in \mathbb{R}^{d_z}$（$d_z = 32$），在隐空间执行流匹配，再解码恢复动作序列。

\subsubsection{VAE 结构}

\textbf{编码器} $q_\phi(\mathbf{z} | \mathbf{a}) = \mathcal{N}(\bm{\mu}, \text{diag}(\bm{\sigma}^2))$：将动作块 $\mathbf{a} \in \mathbb{R}^{T \times D_a}$ 展平后经两层 MLP 输出均值 $\bm{\mu}$ 和方差 $\bm{\sigma}^2$。

\textbf{解码器} $p_\theta(\mathbf{a} | \mathbf{z})$：将隐变量 $\mathbf{z}$ 经两层 MLP 展开为 $\mathbb{R}^{T \times D_a}$。
...
\begin{equation}
  ...
  \label{eq:vae}
\end{equation}
其中 $\mathcal{L}_{\text{recon}} = \|\mathbf{a} - \hat{\mathbf{a}}\|_2^2$ 为重建损失，$\mathcal{L}_{\text{KL}} = D_{\text{KL}}(q_\phi \| \mathcal{N}(0,\mathbf{I}))$ 为 KL 散度正则，$\beta = 0.001$（$\beta$-VAE 设定）。

\subsubsection{隐空间流匹配}

训练时，将编码得到的隐变量 $\mathbf{z}$ 作为流匹配的目标，Action Transformer 学习在时间步 $t \in [0,1]$ 预测速度场 $v_t$：
\begin{equation}
  \mathcal{L}_{\text{FM}} = \mathbb{E}_{t, \mathbf{z}_0 \sim \mathcal{N}(0,\mathbf{I}), \mathbf{z}_1 \sim q_\phi} \bigl\|v_\theta\bigl(\mathbf{z}_t, t, \mathbf{c}\bigr) - (\mathbf{z}_1 - \mathbf{z}_0)\bigr\|_2^2
  \label{eq:fm}
\end{equation}
其中 $\mathbf{z}_t = (1-t)\mathbf{z}_0 + t\mathbf{z}_1$ 为线性插值，$\mathbf{c}$ 为视觉语言条件（融合后的 VLM 特征）。推理时，从 $\mathbf{z}_0 \sim \mathcal{N}(0,\mathbf{I})$ 出发，Euler 积分 $t: 1 \to 0$（10 步），得到 $\hat{\mathbf{z}}_1$，再由解码器恢复动作序列 $\hat{\mathbf{a}}$。
